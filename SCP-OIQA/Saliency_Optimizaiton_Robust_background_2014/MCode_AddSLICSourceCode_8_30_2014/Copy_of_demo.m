...
SRC = 'F:\OIQA\saliency';       %Path of input images
BDCON = 'Data1\BDCON';   %Path for saving bdCon feature image
RES = 'Data1\Res';       %Path for saving saliency maps
...
for num=3:length(file)
    ...
    for x=3:length(angle)
        ...
        for k=1:length(files)
            ...
            srcImg = imread(fullfile(SRC,filename,viewport, srcName));
            if doFrameRemoving
            ...
            pixNumInSP = 400;                           %pixels in each superpixel
            ...
            smapName=fullfile(SRC,filename,viewport, strcat(noSuffixName, '_rbd.jpg'));
