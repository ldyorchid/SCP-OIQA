...
from vit import configs as configs
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.hybrid:
            x = self.hybrid_model(x)        # (b, 1024, 14, 14)
        if x.shape[-1] >= 64:
...
class VisionTransformer(nn.Module):
    def __init__(self, config, num_classes=21843, zero_head=False, vis=False):
        ...
        self.num_classes = num_classes           # num_classes: 10
        self.zero_head = zero_head               # zero_head: True
        self.classifier = config.classifier      # classifier: 'token'

        self.transformer = Transformer(config, vis)
        self.head = Linear(config.hidden_size, num_classes)   # Linear( 768, 10)

    def forward(self, x, labels=None):   # x: (4, 3, 32, 32)   labels: (4, 768)
        x, attn_weights = self.transformer(x) # x: (4, 197, 768)
        logits = self.head(x[:, 0]) # logits: (4, 10)

        if labels is not None:
            loss_fct = CrossEntropyLoss()
            tmp1 = logits.view(-1, self.num_classes)  # tmp1：(4, 10)
            tmp2 = labels.view(-1)                    # tmp2: (4)
